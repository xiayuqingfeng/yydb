...
var ArticleCateObject = {
		...
		$(document).on("click", ".pf-delete-cate-btn", function() {
			...
			showConfirmDialog(function() {
				$.ajax({
					...
					dataType : "json",
					url : "/admin/articles/cate/delete.json",
	...
	_reg_update_cate_btn : function() {
		$(document).on("click", ".pf-update-cate-btn", function() {
			...
			$("#pf-form-cate-save input[name='remark']").val(data.remark);
			$("#pf-form-cate-save input[name='pinyin']").val(data.pinyin);
		});
	},
	// 添加子分类按钮
	_reg_add_sub_cate_btn : function() {
		$(document).on("click", ".pf-add-sub-cate-btn", function() {
			...
			// 显示弹窗
			$("#pf-modal-title").html("添加分类信息，父分类：" + data.name);
			$("#pf-modal-add-cate").modal("show");
		});
	},
	// 初始化应用表格
	_init_app_table : function() {
		$("#pf-table-cate")
				.bootstrapTable(
						{
							method : 'get',
							url : "/admin/articles/cate/all.json?type=" + type,
							cache : false,
							// height : 400,
